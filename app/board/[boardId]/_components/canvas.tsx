...
import { CanvasState, CanvasMode, Camera, Color, LayerType } from "@/types/canvas";
...
liveLayers.set(id,new LiveObject(penPointsToPathLayer(pencilDraft,lastUsedColor)),
    ...
    const translateSelectedLayers = useMutation(({storage,self},point:Point) => {
        ...
        const offset ={ x:point.x - canvasState.current.x,
   ...
   translateSelectedLayers(current);
    ...
    }, [canvasState,resizeSelectedLayer,translateSelectedLayers,camera,continueDrawing,updateSelectionNet,startMultiSelection]);
...
const onPointerDown = useCallback
    ...
    const onPointerUp = useMutation(({ }, e) => {
        ...
        history.resume();
    }, [camera, history, canvasState, insertLayer, unselectLayers,insertPath,setCanvasState]);
    ...
    return (
        <main className="h-full w-full relative bg-neutral-100 touch-auto">
            ...
            <svg className="h-[100vh] w-[100vw]" onWheel={onWheel} onPointerMove={onPointerMove} onPointerLeave={onPointerLeave} onPointerUp={onPointerUp} onPointerDown={onPointerDown}>
                <g style={{ transform: `translate(${camera.x}px, ${camera.y}px)` }}>
                    ...
                    {pencilDraft != null && pencilDraft.length > 0 && (
                        <Path points={pencilDraft} fill={colorToCss(lastUsedColor)} 
                        x={0}
                        y={0}/>
                        )}
                </g>
